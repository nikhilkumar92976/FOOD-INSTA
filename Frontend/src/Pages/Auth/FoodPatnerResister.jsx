...
const FoodPartnerRegister = () => {
  ...
  const handleSubmit = (e) => { 
    ...
    const contact = e.target.contactName.value;
    const phone = e.target.phone.value;
    const email = e.target.email.value;
    const password = e.target.password.value;
    const address = e.target.address.value;

    axios.post("https://food-insta-3.onrender.com/api/auth/register/foodpatner", {
      name:businessName,
      contact,
      phone,
      email,
    ...
    <div className="auth-page-wrapper">
      <div className="auth-card" role="region" aria-labelledby="partner-register-title">
        ...
        <nav className="auth-alt-action" style={{marginTop: '-4px'}}>
          <strong style={{fontWeight:600}}>Switch:</strong> <Link to="/user/register">User</Link> • <Link to="/foodpatner/register">Food partner</Link>
        </nav>
        ...
        <div className="auth-alt-action">
          Already a partner? <Link to="/foodpatner/login">Sign in</Link>
        </div>
      </div>
    </div>
  );
